...
3. In the new window opened, click on `create fork`

    ![create fork](./z.githubDesktopTutorials/4.png)

4. Now a copy/clone of our task repository has been created in your github account.

> Now it's time to install Github Desktop.
...
After Installing and opening the app, you'll see an interface like this.
...
> WeYou must connect Github Desktop with your Github account to continue with this task.

Let's see How,
1. Open Github Desktop and click on `Clone a repository from the internet` button. (First option in the below image)
![Clone a repository from the internet button](./z.githubDesktopTutorials/5.png)

2. A pop-up will show up. Select the first tab `Github.com` and click on `sign in`.
...
4. Now you'll be redirected to your browser. If you have login to your github account, you'll see a popup like this. If not, then login to your github account and click on the `open github desktop` option in the pop-up.
![click on the open github desktop option in the pop-up](./z.githubDesktopTutorials/8.png)

4. Now you'll be redirected to Github Desktop and you could see all your repositories there.
...
Let's see How,
1. After connecting your Github Account with Github desktop, you can see a pop-up like the previous image. In the search bar there search for `Github-Enablment-Task` and select that repository.( Make sure you had <a href="#forkRepo">forked the repository</a>)
![search for Github-Enablment-Task and select that repository](./z.githubDesktopTutorials/10.png)

> If you lost that pop-up, then press `ctrl+shift+o` or goto `files` in the menubar and select `clone a repository` option to open that pop-up again.
...
## The Assessment
> ***The Assessment*** :  **Let’s write a book!** You’re building a book app. It has the feature for the author to add a book. We’ll be working on that feature of the app today. (Don’t worry we’ll be using text files for now). The aim of this task is to get you familiarise with basic Git and Github operations.
...
> And just like that, your first commit is a success. In the `history` section, you can see all the commits made in the repository.In the top you can see the commit that you have done just now.
...
## Unstaging changes
> You just realised that you don’t need to add the change made to `list_all_books.txt` now. Since, `list_all_books.txt` is another feature, you decide to complete developing it on another branch. So you should `unstage` our previous commit.
Let's see How,
1. Open Github desktop
...
6. <p id="checkoutBranch">Now let's switch to `main` branch. (Checkout to `main` branch)</p>
Let's see How,
...
9. Open the ` list_all_books.txt` file and enter `List all books will list all books in ascending order.` into it. Now the `list_all_books.txt` file will have 2 lines. Save the file.
...
> Now, it's time to push your changes and conrtibute to the `GTech µLearn Github repository`, you need to raise a Pull Request Let's see How. Before that remember that you now have two different branches, namely, `develop` and `feature`. The commits you've made so far are in these two branches. Let’s merge them together before pushing.
...
9. After your branch is published, press `ctrl+R` and you'll be redirected to your browser. (If not open this link <pre>github.com/gtech-mulearn/Github-Enablment-Task/compare/main...**USERNAME**:Github-Enablment-Task:develop</pre> Before opening, replace `USERNAME` with your github username Eg: <pre>github.com/gtech-mulearn/Github-Enablment-Task/compare/main...christo-zero-john:Github-Enablment-Task:develop</pre>).
